...
int main()
{
	...
	*/
	int numberOfShoes = 0;
	string *shoeList = nullptr;

	// Welcome message
	cout << "\t\tWelcome to the Simple Shoe Sorter!" << endl;
	cout << "\tWith this program, you will be able to keep track of your shoes and even" << endl;
	cout << "sort them alphabetically! Please choose one of the options below to get started" << endl;
	mainMenu(numberOfShoes, shoeList);
...
void mainMenu(int &numberOfShoes, string *shoeList)
{
	...
	cout << "2. Import previously created list" << endl;
	...
	do
	{
		...
		case 1:
			...
			allocateData(numberOfShoes, shoeList);
			addShoes(numberOfShoes, shoeList);
			switchMenu(numberOfShoes, shoeList);
			break;
		case 2:
			allocateData(numberOfShoes, shoeList);
			readInput(numberOfShoes, shoeList);
			switchMenu(numberOfShoes, shoeList);
			break;
		case 3:
			exit(0);
...
void readInput(int &numberOfShoes, string *shoeList)
{
	...
	do
	{
		cout << "Enter the name that your previous list is stored under: ";
		getline(cin, name);
		getline(cin, name);

		cout << "Checking for file..." << endl;

		inputList.open(name + "'sShoeFile.txt");

		if (inputList.is_open())
		{
			while (!inputList.eof())
			{
				getline(inputList, temp);
				count += 1;
			}
			inputList.close();
			inputList.open(name + "'sShoeFile.txt");
			...
			while (getline(inputList, shoeList[count]))
			{
				getline(inputList, shoeList[count]);
				cout << shoeList[count] << endl;
				count++;
			}
		...
		else
		{
			do
			{
				cout << "No file is found under that name. Would you like to try again? (Y/N)" << endl;
				cin >> yesNo;
				...
				{
					cout << "Please enter a Y/N response" << endl;
				}
			} while (yesNo != "y" && yesNo != "Y" && yesNo != "n" && yesNo != "N");
		}
	} while (inputList.fail() && (yesNo != "n" || yesNo != "N"));
...
// Function to allocate data for dynamic array shoeList
void allocateData(int numberOfShoes, string *shoeList)
{
